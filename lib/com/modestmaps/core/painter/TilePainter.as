package com.modestmaps.core.painter
{
	...
	public class TilePainter extends EventDispatcher implements ITilePainter
	{
		...
		protected static const DEFAULT_SAMPLE_EDGES:Boolean = false; // !!! only enable this if you have crossdomain permissions to access Loader content
		...
		/** set this to true to copy edge pixels from tiles into a 1px buffer - requires crossdomain.xml permissions so won't work online with most providers */
		public static var sampleEdges:Boolean = DEFAULT_SAMPLE_EDGES;
		...
		private function onLoadEnd(event:Event):void
		{
			...
			if (smoothContent || sampleEdges) {
				try {
					var bitmap:Bitmap = loader.content as Bitmap;
					if (sampleEdges) {
						//var t:Number = getTimer();
						//trace('copying edge pixels');
						// add a 1px border to the tile
						var bmd:BitmapData = new BitmapData(bitmap.width+2, bitmap.height+2, true, 0xffffffff);
						bmd.lock();
						bmd.copyPixels(bitmap.bitmapData, bitmap.bitmapData.rect, new Point(1,1));
						// edges
						bmd.copyPixels(bitmap.bitmapData, new Rectangle(0,0,1,bitmap.height), new Point(0,1));
						bmd.copyPixels(bitmap.bitmapData, new Rectangle(bitmap.width-1,0,1,bitmap.height), new Point(bitmap.width+1,1));
						bmd.copyPixels(bitmap.bitmapData, new Rectangle(0,0,bitmap.width,1), new Point(1,0));
						bmd.copyPixels(bitmap.bitmapData, new Rectangle(0,bitmap.height-1,bitmap.width,1), new Point(1,bitmap.height+1));
						// corners
						bmd.copyPixels(bitmap.bitmapData, new Rectangle(0,0,1,1), new Point(0,0));
						bmd.copyPixels(bitmap.bitmapData, new Rectangle(bitmap.width-1,bitmap.height-1,1,1), new Point(bitmap.width+1,bitmap.height+1));
						bmd.copyPixels(bitmap.bitmapData, new Rectangle(bitmap.width-1,0,1,1), new Point(bitmap.width+1,0));
						bmd.copyPixels(bitmap.bitmapData, new Rectangle(0,bitmap.height-1,1,1), new Point(0,bitmap.height+1));
						bmd.unlock();
						bitmap.bitmapData = bmd;
						bitmap.x = bitmap.y = -1;
						//trace("cloned edges in", getTimer()-t, 'ms');
					}
					if (smoothContent) {
						//trace('setting smoothing to true');
