package flare.util {
	...
	public class Displays {
		...
		public static function addStageListener(d : DisplayObject,
			...
			if (d.stage) {
				d.stage.addEventListener(eventType, listener, useCapture, priority, useWeakReference);
				return null;
			} else {
		...
		public static function sortChildren(
			...
			// Array.NUMERIC
			if (cmp == null) a.sort(Function(Array.NUMERIC)) else a.sort(cmp);
		...
		public static function setChildrenProperties(d : DisplayObjectContainer,
			...
			for (var name:String in vals) {
				var value : * = vals[name];
				var v : Function = value is Function ? value as Function : value is IEvaluable ? IEvaluable(value).eval : null;
